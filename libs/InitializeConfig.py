...
class EnvironmentConfig:
    ...
    def __post_init__(self):
        ...
        if not self.uuid:
            raise ConfigurationException("UUID cannot be empty")
        
        if not self.billing_group_id:
            raise ConfigurationException("Billing group ID cannot be empty")
...
class ModuleConfigLoader(ConfigLoader):
    ...
    def load(self, environment: str, member: str) -> EnvironmentConfig:
        ...
        if hasattr(config_module, "test_config"):
            config_data = getattr(config_module, "test_config")
        elif hasattr(config_module, "config"):
            # Backward compatibility with legacy name
            config_data = getattr(config_module, "config")
        else:
            raise ConfigurationException(
                f"Configuration module {module_name} must define 'test_config' or 'config'"
            )
...
class DefaultManagerFactory:
    ...
    def __init__(self, client: Optional[BillingAPIClient] = None):
        """
...
class EnvironmentPreparer:
    ...
    def prepare(self) -> PaymentStatus:
        ...
        try:
            ...
            elif payment_status == PaymentStatus.READY:
                logger.info("Payment is READY, changing to REGISTERED")
                self._payment_manager.change_payment_status(payment_group_id)
                return PaymentStatus.REGISTERED
                
            elif payment_status == PaymentStatus.REGISTERED:
...
class ConfigurationManager:
    ...
    def validate_config(self, config: EnvironmentConfig) -> None:
        ...
        if isinstance(bg_id, str) and bg_id.startswith("test_") and not config.campaign_id:
            logger.warning(
                "Test billing group detected but no campaign IDs configured"
            )
        elif isinstance(bg_id, list) and any(id.startswith("test_") for id in bg_id) and not config.campaign_id:
            logger.warning(
                "Test billing group detected but no campaign IDs configured"
            )
