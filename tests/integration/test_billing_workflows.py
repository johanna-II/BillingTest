...
from libs.constants import (
    AdjustmentType, AdjustmentTarget, BatchJobCode, 
    CounterType, PaymentStatus, CreditType
)
...
class TestBillingWorkflows:
    ...
    def managers(self, api_client, month, member):
        ...
        return {
            ...
            'contract': ContractManager(
                month=month, 
                billing_group_id=f"bg-{member}-001"
            ),
            'calculation': CalculationManager(
                month=month,
                uuid=f"uuid-{member}-001"
            ),
    ...
    def test_standard_billing_cycle(self, managers):
        ...
        contract_result = managers['contract'].apply_contract(
            contract_id="standard-contract-001",
            name="Standard Monthly Contract"
        )
    ...
    def test_billing_with_credits(self, managers):
        ...
        # 3. Use coupon
        coupon_result = managers['credit'].use_coupon(
    ...
    def test_batch_operations(self, managers):
        ...
        status = managers['batch'].get_batch_status(
            BatchJobCode.BATCH_CREDIT_EXPIRY
        )
    ...
    def test_end_to_end_monthly_billing(self, managers):
        ...
        managers['batch'].request_batch_job(
            BatchJobCode.BATCH_GENERATE_STATEMENT
        )
        ...
        try:
            ...
            with pytest.raises(Exception):
                managers['contract'].apply_contract(
                    contract_id="invalid-contract-xxx"
                )
