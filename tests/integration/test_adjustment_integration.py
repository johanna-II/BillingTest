...
from libs.constants import AdjustmentType, AdjustmentTarget, CounterType, PaymentStatus
...
class TestAdjustmentIntegration:
    ...
    def adjustment_context(self, api_client, month, test_billing_group):
        ...
        return {
            ...
            'calculation': CalculationManager(
                month=month, 
                uuid=f"uuid-{test_billing_group}"
            ),
    ...
    def test_adjustment_deletion_effect(self, adjustment_context, test_app_keys):
        ...
        ctx['calculation'].recalculate_all()
        pg_id, _ = ctx['payment'].get_payment_status()
        amount_with_adjustment = ctx['payment'].check_unpaid_amount(pg_id) if pg_id else 0
        ...
        ctx['calculation'].recalculate_all()
        amount_without_adjustment = ctx['payment'].check_unpaid_amount(pg_id) if pg_id else 0
