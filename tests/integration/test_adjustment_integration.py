...
class TestAdjustmentIntegration(BaseIntegrationTest):
    """Integration tests for adjustment functionality."""

    def test_adjustment_impact_on_billing(self, test_context, test_app_keys) -> None:
        ...
        initial_amount = managers["payment"].check_unpaid_amount(pg_id) if pg_id else 0
        ...
        pg_id, _ = managers["payment"].get_payment_status()
        adjusted_amount = managers["payment"].check_unpaid_amount(pg_id) if pg_id else 0
    ...
    def test_percentage_adjustment_calculation(
        ...
        managers["calculation"].recalculate_all()
        pg_id, _ = managers["payment"].get_payment_status()
        base_amount = managers["payment"].check_unpaid_amount(pg_id) if pg_id else 0
        ...
        managers["calculation"].recalculate_all()
        discounted_amount = (
            managers["payment"].check_unpaid_amount(pg_id) if pg_id else 0
        )
    ...
    def test_multiple_adjustments_stacking(self, test_context, test_app_keys) -> None:
        ...
        all_adjustments = managers["adjustment"].get_adjustments(
            adjustment_target=AdjustmentTarget.BILLING_GROUP,
            target_id=test_context["billing_group_id"],
        )
        ...
        pg_id, _ = managers["payment"].get_payment_status()
        final_amount = managers["payment"].check_unpaid_amount(pg_id) if pg_id else 0
        assert final_amount >= 0

    def test_adjustment_deletion_effect(self, test_context, test_app_keys) -> None:
        ...
        if adj_id:
            delete_result = managers["adjustment"].delete_adjustment(
                adjustment_ids=adj_id, adjustment_target=AdjustmentTarget.BILLING_GROUP
            )
            self.assert_api_success(delete_result, "DELETED")
    ...
    @pytest.mark.slow
    def test_adjustment_pagination(self, test_context) -> None:
        ...
        if all_adjustments:
            managers["adjustment"].delete_adjustment(
