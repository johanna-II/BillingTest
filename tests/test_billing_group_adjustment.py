...
class TestAdjustmentOnly:
    ...
    def test_bgAdjTC1(self):
        ...
        expect_result = (statements['charge'] - 1000) + math.floor((statements['charge'] - 1000) * 0.1)
    ...
    def test_bgAdjTC2(self):
        adjObj = adj.Adjustments(self.config.month)
        adjObj.applyAdjustment(adjustmentTarget="BillingGroup", billingGroupId=self.config.billing_group_id[0],
                               adjustmentType="PERCENT_DISCOUNT", adjustment=5)
        calcObj = calc.Calculation(self.config.month, self.config.uuid)
        calcObj.recalculationAll()
        # 결제 후 금액 비교
        statements, total_payments = self.config.commonTest()
    ...
    def test_bgAdjTC3(self):
        adjObj = adj.Adjustments(self.config.month)
        adjObj.applyAdjustment(adjustmentTarget="BillingGroup", billingGroupId=self.config.billing_group_id[0],
                               adjustmentType="STATIC_EXTRA", adjustment=5000)
        calcObj = calc.Calculation(self.config.month, self.config.uuid)
        calcObj.recalculationAll()
        # 결제 후 금액 비교
        statements, total_payments = self.config.commonTest()
        expect_result = (statements['charge'] + 5000) + math.floor((statements['charge'] + 5000) * 0.1)
        self.config.verifyAssert(statements=statements['totalAmount'], payments=total_payments, expected_result=expect_result)
