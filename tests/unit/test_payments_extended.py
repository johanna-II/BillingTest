...
from libs.Payments import PaymentManager, PaymentValidator, PaymentStatement
...
class TestPaymentsExtended:
    ...
    def test_get_payment_status_api_exception(self):
        ...
        self.mock_client.get_statements_console.side_effect = APIRequestException("API error")
    ...
    def test_change_payment_status_api_exception(self):
        ...
        self.mock_client.change_status.side_effect = APIRequestException("Change failed")
        
        with pytest.raises(APIRequestException) as exc_info:
            self.payment_manager.change_payment_status("pg-123", PaymentStatus.REGISTERED)
    ...
    def test_prepare_payment_unknown(self):
        ...
        self.mock_client.get_statements_console.return_value = {
            "statements": [{
                "paymentGroupId": "pg-123",
                "paymentStatusCode": "UNKNOWN_STATUS"
            }]
        }
    ...
    def test_validate_payment_amount(self):
        ...
        assert self.payment_manager.validate_payment_amount(50, min_amount=10, max_amount=100) is True
        assert self.payment_manager.validate_payment_amount(5, min_amount=10, max_amount=100) is False
    ...
    def test_retry_failed_payment(self):
        ...
        self.mock_client.retry_payment.assert_called_once_with(payment_id="pay-123", retry_count=1)
        ...
        assert result == {"status": "SUCCESS"}
        assert self.mock_client.retry_payment.call_count == 4  # 1 from previous + 3 from this test
    ...
    def test_process_batch_payments(self):
        ...
        self.mock_client.process_batch_payments.assert_called_once_with(payment_requests)
    ...
    def test_make_payment_all_retries_fail(self):
        ...
        self.mock_client.make_payment.side_effect = APIRequestException("Payment failed")
        
        with pytest.raises(APIRequestException):
            self.payment_manager.make_payment("pg-123", retry_on_failure=True, max_retries=2)
    ...
    def test_cancel_payment_api_exception(self):
        ...
        self.mock_client.cancel_payment.side_effect = APIRequestException("Cancel failed")
