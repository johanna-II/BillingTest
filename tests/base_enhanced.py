...
from libs.dependency_injection import configure_dependencies
...
class TestContext:
    ...
    _metering_manager: MeteringManager | None = None
    _credit_manager: CreditManager | None = None
    _contract_manager: ContractManager | None = None
    _batch_manager: BatchManager | None = None
    _calculation_manager: CalculationManager | None = None
    _adjustment_manager: AdjustmentManager | None = None
    ...
    def credit_manager(self) -> CreditManager:
        ...
        if self._credit_manager is None:
            ...
            if hasattr(self.config, "campaign_id"):
                ...
                self._credit_manager.give_campaign_id = getattr(
                    self.config, "give_campaign_id", None
                )
                self._credit_manager.paid_campaign_id = getattr(
                    self.config, "paid_campaign_id", None
                )
        return self._credit_manager

    @property
    def contract_manager(self) -> ContractManager | None:
...
class EnhancedBaseBillingTest:
    ...
    def setup_test_context(self, env: str, member: str, month: str) -> TestContext:
        ...
        self.context = TestContext(
            env=env, member=member, month=month, uuid=test_uuid, config=config
        )
    ...
    def send_bulk_metering(self, count: int = 10, base_volume: float = 100) -> None:
        ...
        for i in range(count):
            self.send_metering_data(
                counter_name=f"test.meter.{i}", counter_volume=base_volume + i
            )
    ...
    def setup_credit(self, amount: int = 10000, credit_type: str = "FREE") -> str:
        ...
        return self.context.credit_manager.give_credit(
            amount=amount, credit_type=credit_type
        )

    def create_contract(
        self, contract_type: str = "VOLUME", discount_rate: float = 10.0
    ) -> dict[str, Any]:
        ...
        return self.context.contract_manager.create_contract(
            contract_type=contract_type, discount_rate=discount_rate
        )
    ...
    def verify_billing_amount(
        self, expected_amount: float, tolerance: float = 0.01
    ) -> None:
    ...
    def apply_adjustment(
        self, adjustment_type: str, amount: float, reason: str = "Test adjustment"
    ) -> dict[str, Any]:
        """Apply an adjustment."""
