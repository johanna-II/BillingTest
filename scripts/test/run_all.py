...
def run_category(category: str, args: List[str], project_root: Path) -> Tuple[bool, float]:
...
Examples:
  ...
  python scripts/test/run_all.py --unit-args "--parallel 2"
    ...
    parser.add_argument(
        '--only',
        help='Comma-separated list of categories to run (skip all others)'
    )
    ...
    for category, _, _ in TEST_STAGES:
        parser.add_argument(
            f'--{category}-args',
            help=f'Override arguments for {category} tests'
        )
