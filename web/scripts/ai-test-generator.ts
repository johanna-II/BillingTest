...
/**
 * NOTE: Using async IIFE instead of top-level await
 *
 * Reason: tsx executes in CJS mode by default, which doesn't support top-level await
 * Error: "Top-level await is currently not supported with the 'cjs' output format"
 *
 * SonarQube S7785 warning accepted: Runtime compatibility > code style preference
 * This is a pragmatic trade-off for cross-environment execution
 */
void (async function mainExecution() {
  try {
    await main();
  } catch (error) {
